...
const MAKE_TOOL = {
  name: 'make',
  description: "Operate the user's Make.com account. actions: 'list_scenarios' (optional `query` filters by name); 'get_blueprint' (needs scenario_id — COMPACT module map for an OVERVIEW; long values shortened); 'get_module' (needs scenario_id + module_id — ONE module's FULL raw JSON: body, prompt, every parameter/mapper/metadata — use before editing); 'clone_scenario' (needs scenario_id = the SOURCE to copy + optional `name` for the new one — copies an existing WORKING scenario exactly; THIS IS THE RELIABLE WAY TO BUILD, then customize with update_module); 'update_module' (needs scenario_id + module_id + `parameters` and/or `mapper` objects to merge in — edits one module of an existing scenario); 'create_scenario' (needs `name` + a full `blueprint` object — only for genuinely novel simple flows; prefer clone_scenario); 'run_scenario'/'activate'/'deactivate' (need scenario_id); 'check_run' (needs scenario_id — returns whether the LAST execution SUCCEEDED or FAILED; ALWAYS use this to VERIFY after building/editing, and to diagnose errors like a Google Sheets 403); 'list_connections' (returns each connection's id + account EMAIL — use to pick the right account); 'list_data_stores'.",
  input_schema: {
    type: 'object',
    properties: {
      action: { type: 'string', enum: ['list_scenarios', 'get_blueprint', 'get_module', 'clone_scenario', 'update_module', 'create_scenario', 'run_scenario', 'activate', 'deactivate', 'check_run', 'list_connections', 'list_data_stores'] },
...
const SUBAGENTS = {
  ...
  api:       { name: 'Wire',
    system: "You are Wire, an API/automation specialist AND the user's Make.com expert. For automation/API tasks give concrete steps, code, or payloads. For Make.com: BUILD scenarios (emit a valid blueprint object and call make.create_scenario), STUDY them (make.get_blueprint for the module-by-module overview, then explain in plain English), and list/run/activate scenarios with the `make` tool. Always name a created scenario clearly and confirm what it will do.\n\nIMPORTANT: get_blueprint gives a COMPACT map where long values (HTTP request bodies, GPT/AI prompts, mapped fields) are shortened with '…'. When the user asks about, quotes, or wants to EDIT a specific module's exact content, ALWAYS call make.get_module with that scenario_id + module_id first to read the FULL untruncated config. NEVER fabricate or guess a body, prompt, saved-search id, or field value — if you haven't fetched it with get_module, fetch it.\n\nWHEN BUILDING A NEW AUTOMATION, the RELIABLE way is to CLONE an existing working scenario and customize it — do NOT hand-write a full blueprint from scratch (that fails, because you can't know the exact module identifiers, versions, connection ids, and metadata). Workflow: (1) make.list_scenarios to find the closest existing template; (2) make.clone_scenario with scenario_id = that source + a new `name`; (3) make.get_module on the modules you must change to read their real config; (4) make.update_module with the new `parameters`/`mapper` values; (5) VERIFY, then report the REAL new scenario name and id. Do everything in this SAME turn — never promise to build 'module by module' later. Only use create_scenario (hand-written blueprint) for a genuinely simple, novel flow when NO similar scenario exists.\n\nCRITICAL — CLONING CHANGES NOTHING BUT THE NAME. After clone_scenario you MUST apply EVERY change the user asked for with update_module, or the new scenario still points at the SOURCE's sheets/lists/accounts and will fail. In particular:\n• To repoint a Google Sheets module to a spreadsheet the USER gave you: update_module with mapper { \"from\": \"drive\", \"spreadsheetId\": \"<the id from the user's sheet URL>\" } — a Sheets URL .../spreadsheets/d/<ID>/edit means spreadsheetId=<ID>. Do this for EVERY google-sheets module (read AND write) in the scenario, and keep/confirm the tab (sheetId) exists in the new sheet.\n• The connection matters: a cloned Sheets module keeps the SOURCE account's connection, which usually cannot access the user's own new sheet → Google 403 PERMISSION_DENIED. Call make.list_connections (it shows each connection's account EMAIL), pick the connection whose account owns/can-edit the target sheet (for the user's OWN sheet that's THEIR Google account), and set it via update_module parameters { \"__IMTCONN__\": <that connection id> }.\n• Change any HTTP/API bodies too (e.g. a GovSpend saved-search id) with update_module.\nALWAYS VERIFY before saying it's done: after editing, run it (or if it is scheduled/already ran) call make.check_run — if the last run FAILED (e.g. a Sheets 403), fix the connection or ask the user to share the sheet, and report the REAL status. NEVER claim a scenario works without check_run confirming SUCCESS. If any step errors or returns BUILD_INCOMPLETE, report the exact reason — never claim success you didn't get.\n\n" + MAKE_KNOWLEDGE,
    tools: [MAKE_TOOL] },
  files:     { name: 'Sift',
...
async function makeAction(inp) {
  ...
  if (a === 'list_connections') {
    const team = await makeTeam();
    const r = await makeCall('GET', '/connections?teamId=' + team);
    const list = Array.isArray(r.connections) ? r.connections : (Array.isArray(r) ? r : []);
    const out = [];
    for (const c of list) {
      let email = '';
      try { const d = await makeCall('GET', '/connections/' + c.id); const dd = (d && d.connection) || d; if (dd && dd.metadata && dd.metadata.type === 'email') email = dd.metadata.value; } catch (_) {}
      out.push('#' + c.id + ' ' + (c.accountName || '') + (c.accountLabel ? ' (' + c.accountLabel + ')' : '') + (email ? ' — account: ' + email : ''));
    }
    return 'Connections (use the id as __IMTCONN__; match the account email to whose files you need):\n' + out.join('\n');
  }
  if (a === 'check_run') {
    const r = await makeCall('GET', '/scenarios/' + inp.scenario_id + '/logs?pg[limit]=3');
    const arr = r.scenarioLogs || r.logs || [];
    if (!arr.length) return 'No runs yet for scenario ' + inp.scenario_id + ' — it has not executed.';
    const label = (s) => (s === 1 ? 'SUCCESS' : (s === 2 ? 'ERROR' : ('status ' + s)));
    const lines = arr.slice(0, 3).map((l) => label(l.status) + ' @ ' + (l.timestamp || '?') + (l.operations != null ? ' (' + l.operations + ' ops)' : ''));
    const latest = arr[0];
    const verdict = latest.status === 1
      ? 'The most recent run SUCCEEDED.'
      : (latest.status === 2 ? 'The most recent run FAILED (error). If a Google Sheets module 403 "PERMISSION_DENIED" — the connection\'s Google account cannot access that spreadsheet; switch to a connection whose account owns/can-edit the sheet, or have the user share the sheet with that account.' : 'Most recent run: ' + label(latest.status) + '.');
    return verdict + '\nRecent runs:\n' + lines.join('\n');
  }

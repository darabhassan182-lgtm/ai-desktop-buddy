...
(function () {
  ...
  var CHAR_H_MGR = 2.4;                // Agent Sea towers over the team — the brain of it all
  var CHAR_YAW = Math.PI;              // model forward-facing offset (flip if characters face backward)
  ...
  // Each agent has its OWN model in window.NEXUS_CHARS (a map of id -> base64 GLB).
  // Parse each independently; a body pops in when its parse resolves. Never throws.
  function startCharacterLoad() {
    var models = window.NEXUS_CHARS;
    if (!models || !GLTFLoader || !SkeletonUtils) { warnOnce('[World] character models unavailable — using capsules'); buildFallbackCharacters(); return; }
    var loader; try { loader = new GLTFLoader(); } catch (e0) { buildFallbackCharacters(); return; }
    for (var i = 0; i < IDS.length; i++) parseFor(IDS[i]);
    function parseFor(id) {
      var b64 = models[id];
      if (!b64) { buildFallbackBody(id); return; }
      var buf; try { buf = b64buf(b64); } catch (e) { buildFallbackBody(id); return; }
      try {
        loader.parse(buf, '', function (gltf) {
          try { buildCharacterBody(id, gltf, (gltf && gltf.animations) || []); }
          catch (e2) { buildFallbackBody(id); }
        }, function () { buildFallbackBody(id); });
      } catch (e3) { buildFallbackBody(id); }
    }
  }

  // Clone one agent's own model, KEEP its textures (cute blocky people), normalize
  // scale/feet/facing, wire the mixer. Agent Sea (manager) glows with her accent — the brain.
  function buildCharacterBody(id, gltf, clips) {
    ...
    clone.traverse(function (o) {
      if (o && (o.isMesh || o.isSkinnedMesh)) {
        if (o.material && o.material.clone) { try { o.material = o.material.clone(); } catch (e) {} }
        if (isMgr && o.material) { try { o.material.emissive = new THREE.Color(A.accent); o.material.emissiveIntensity = 0.4; } catch (e) {} }
        o.castShadow = true; o.receiveShadow = true; o.frustumCulled = false;
        if (o.material) ud.bodyMats.push(o.material);
      }
    });

    // Accurate scale: these blocky models are NON-skinned node rigs, so Box3.setFromObject
    // is reliable once world matrices update. Scale to target height; drop feet to y=0.
    clone.updateWorldMatrix(true, true);
    var bb = new THREE.Box3().setFromObject(clone);
    var srcH = (bb.max.y - bb.min.y) || 1;
    clone.scale.setScalar((isMgr ? CHAR_H_MGR : CHAR_H) / srcH);
    clone.updateWorldMatrix(true, true);
    bb.setFromObject(clone);
    clone.position.y = -bb.min.y;
    clone.rotation.y = CHAR_YAW;
    ud.rig.add(clone);
    ud.model = clone;
    ...
    if (walkClip) {
      ...
      ud.walk.play(); ud.walk.setEffectiveWeight(0);
    }
    ud.walking = false;
    ud.built = true;
  }
